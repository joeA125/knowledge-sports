...
> ### ⚠️ And Then Someone Did It — Narrowly, 2026-08-29
>
> [[off-ball-defensive-performance-blame|Bischofberger et al. (2026)]] evaluate **tackles won, tackles won ratio and interceptions** against their own ~29 metric variants, on **three shared datasets**, under one protocol combining validity against external anchors with three robustness statistics.
>
> **That is a real cross-framework benchmark**, run on proprietary multi-league data by a group with club and federation access. The structural excuse about licensing is now much weaker.
>
> **The claim survives only on scope:** the comparison is new metrics against traditional counting stats, not [[obso|OBSO]] against [[c-obso|C-OBSO]] against [[space-occupation-gain|SOG]]. **None of the vault's six off-ball mechanisms appears in any comparison, by anyone.**
>
> The result is worth carrying regardless: *interceptions per 90* scores **−0.3** on their validity scale and *tackles won per 90* **−0.6**. The two most widely used defensive metrics in football come out close to uninformative about defensive positioning — which bears directly on [[rare-event-proxy-targets]] and [[vdep]].
